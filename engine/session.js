...
class Session {
            ...
            if (!vodResponse.type) {
              ...
              if (vodResponse.type === 'gap') {
                loadPromise = new Promise((resolve, reject) => {
                  this._fillGap(null, vodResponse.desiredDuration, vodResponse)
                    .then(gapVod => {
                      currentVod = gapVod;
                      resolve(gapVod);
                    }).catch(reject);
        ...
        try {
          ...
          if (isLeader) {
            ...
            if (!vodResponse.type) {
              ...
              loadPromise = new Promise((resolve, reject) => {
                this._fillGap(currentVod, vodResponse.desiredDuration, vodResponse)
                  .then(gapVod => {
                    newVod = gapVod;
                    resolve(newVod);
                  }).catch(reject);
      ...
      try {
        ...
        slateVod.load()
          .then(() => {
            ...
            const timestamp = opts.unixTs ? opts.unixTs : Date.now();
  ...
  _truncateSlate(afterVod, requestedDuration, vodUri, opts) {
    return new Promise((resolve, reject) => {
      let nexVodUri = null;
      try {
        if (vodUri) {
  ...
  _fillGap(afterVod, desiredDuration, opts) {
    return new Promise((resolve, reject) => {
      ...
      if (desiredDuration > this.slateDuration) {
        ...
        loadSlatePromise = this._loadSlate(afterVod, reps, opts);
        ...
        loadSlatePromise = this._truncateSlate(afterVod, desiredDuration / 1000, null, opts);
