...
class SharedSessionState {
  sessionId: string;
  instanceId: string;
  cache: { currentVod: ICurrentVodCache };
  cacheTTL: number;
  store: SessionStateStore;

  constructor(store: SessionStateStore, sessionId: string, instanceId: string, opts?: { cacheTTL?: number }) {
  ...
  async setCurrentVod(hlsVod: any, opts?: { ttl?: number }): Promise<any> {
    if (!this.sessionId) {
      throw new Error("shared session state store has not been initialized");
    }
    if (this.store.isShared()) {
  ...
  async setValues(keyValues: Record<string, any>): Promise<Record<string, any>> {
    if (await this.store.isLeader(this.instanceId)) {
  ...
  async increment(key: string, inc?: number): Promise<any> {
...
class SessionStateStore extends SharedStateStore {
  // `cache` is re-shaped from the base class's generic cache to a leader cache.
  declare cache: { leader: ILeaderCache };
  // Referenced (as `undefined`) by clearLeaderCache in the original JS; kept
  // as an optional field to preserve behavior without introducing a value.
  instanceId?: string;
  ...
  async clearLeaderCache(): Promise<void> {
  ...
  async create(sessionId: string, instanceId: string): Promise<SharedSessionState> {
