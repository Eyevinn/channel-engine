...
class ChannelEngine {
  constructor(assetMgr, options) {
    ...
    if (options && options.averageSegmentDuration) {
      this.streamerOpts.defaultAverageSegmentDuration =
        options.averageSegmentDuration;
    }
    ...
    if (options && options.playheadDiffThreshold) {
      this.streamerOpts.defaultPlayheadDiffThreshold =
        options.playheadDiffThreshold;
    }
    ...
    this.server.get(
      ...
      this._handleEventStream.bind(this)
    );
    ...
    setInterval(async () => {
      ...
      for (const chId in sessionSwitcher) {
        if (Object.hasOwnProperty.call(sessionSwitcher, chId)) {
          ...
          switchSession[chId] = await switcher.streamSwitcher(
            sessions[chId],
            sessionsLive[chId]
          );
      ...
      //this.streamSwitchTimeIntervalMs = Math.min(...timeIntervalsMs);
      debug(`StreamSwitchTimeIntervalMs=[${this.streamSwitchTimeIntervalMs}]`);
    }, this.streamSwitchTimeIntervalMs);

    const pingSession = setInterval(async () => {
      await this.sessionStore.sessionStateStore.ping(this.instanceId);
    }, 3000);
    const pingSessionLive = setInterval(async () => {
  ...
  async updateChannelsAsync(channelMgr, options) {
    ...
    const newChannels = channelMgr
      .getChannels()
      .filter((channel) => !sessions[channel.id]);
    const addAsync = async (channel) => {
      debug(`Adding channel with ID ${channel.id}`);
      sessions[channel.id] = new Session(
        this.assetMgr,
        {
          sessionId: channel.id,
          averageSegmentDuration:
            channel.options && channel.options.averageSegmentDuration
              ? channel.options.averageSegmentDuration
              : this.streamerOpts.defaultAverageSegmentDuration,
          useDemuxedAudio: options.useDemuxedAudio,
          playheadDiffThreshold:
            channel.options && channel.options.playheadDiffThreshold
              ? channel.options.playheadDiffThreshold
              : this.streamerOpts.defaultPlayheadDiffThreshold,
          maxTickInterval:
            channel.options && channel.options.maxTickInterval
              ? channel.options.maxTickInterval
              : this.streamerOpts.defaultMaxTickInterval,
          targetDurationPadding:
            channel.options && channel.options.targetDurationPadding
              ? channel.options.targetDurationPadding
              : this.streamerOpts.targetDurationPadding,
          forceTargetDuration:
            channel.options && channel.options.forceTargetDuration
              ? channel.options.forceTargetDuration
              : this.streamerOpts.forceTargetDuration,
          profile: channel.profile,
          audioTracks: channel.audioTracks,
          closedCaptions: channel.closedCaptions,
          slateUri:
            channel.slate && channel.slate.uri
              ? channel.slate.uri
              : this.defaultSlateUri,
          slateRepetitions:
            channel.slate && channel.slate.repetitions
              ? channel.slate.repetitions
              : this.slateRepetitions,
          slateDuration:
            channel.slate && channel.slate.duration
              ? channel.slate.duration
              : this.slateDuration,
      ...
      if (!this.monitorTimer[channel.id]) {
        this.monitorTimer[channel.id] = setInterval(async () => {
          await this._monitorAsync(sessions[channel.id]);
        }, 5000);
      }
    ...
    await Promise.all(
      newChannels
        ...
        .concat(newChannels.map((channel) => addLiveAsync(channel)))
    );

    debug(`Have any channels been removed?`);
    const removedChannels = Object.keys(sessions).filter(
      (channelId) => !channelMgr.getChannels().find((ch) => ch.id == channelId)
    );
    ...
    await Promise.all(
      removedChannels.map((channelId) => removeAsync(channelId))
    );
  }
  ...
  start() {
    const startAsync = async (channelId) => {
      ...
      if (!this.monitorTimer[channelId]) {
        this.monitorTimer[channelId] = setInterval(async () => {
          await this._monitorAsync(session);
        }, 5000);
      }
    ...
    const startLiveAsync = async (channelId) => {
      ...
      console.log(
        "(2)(2)(2) HEY IM STARTING -> sessionLive.startPlayheadAsync()"
      );
    ...
    (async () => {
      ...
      await Promise.all(
        Object.keys(sessions)
          ...
          .concat(
            Object.keys(sessionsLive).map((channelId) =>
              startLiveAsync(channelId)
            )
          )
      );
    })();
  ...
  getPlayheadCount() {
    return Object.keys(sessions).filter((sessionId) =>
      sessions[sessionId].hasPlayhead()
    ).length;
  }
  ...
  async _monitorAsync(session) {
    ...
    debug(
      `MONITOR (${new Date().toISOString()}) [${status.sessionId}]: playhead: ${
        status.playhead.state
      }`
    );
    ...
    if (session && sessionLive) {
      try {
        while (switchSession[req.params[1]] === null) {
          debug(
            `[${req.params[1]}]: Waiting for streamSwitcher to finish switching session`
          );
          await timer(500);
        }
        let body = null;
        if (switchSession[req.params[1]]) {
          debug(
            `[${req.params[1]}]: Responding with Altered-Live stream manifest`
          );
          ...
          body = await session.getCurrentMediaManifestAsync(
            req.params[0],
            req.headers["x-playback-session-id"]
          );
        }
